              ...
              <div v-for="(a, i) in actions.slice(0, 8)" :key="a.id" class="prof-timeline-item" :style="{ animationDelay: i * 30 + 'ms' }">
                ...
                <div class="prof-timeline-card">
                  ...
                  <div class="prof-timeline-meta" v-if="a.entity">
                    <span class="meta-badge">{{ a.entity }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="dashboard-side">
          <div class="prof-info-card" style="margin-bottom: 14px;">
            ...
            <div class="quick-rec-form">
              <input v-model="recForm.studentName" class="form-input" placeholder="Nom de l'étudiant" style="margin-bottom: 8px; width: 100%; box-sizing: border-box;" />
              <textarea v-model="recForm.message" class="form-textarea" placeholder="Message de recommandation..." rows="3" style="margin-bottom: 8px; width: 100%; box-sizing: border-box;"></textarea>
              <button class="btn-primary" @click="submitQuickRec" :disabled="sendingRec || !recForm.studentName || !recForm.message" style="width: 100%; justify-content: center;">
          ...
          <div class="prof-info-card">
            ...
            <div v-else class="notif-mini-list">
              <div v-for="n in notifications.slice(0, 4)" :key="n.id" class="notif-mini-item">
                <div class="notif-mini-dot" :class="'dot-' + (n.type ? n.type.toLowerCase() : 'default')"></div>
...
import { getProfessionalNotifications, getProfessionalActionHistory, getProfessionalInternships, getProfessionalProjects, createProfessionalRecommendation } from '@/services/professionalApi'
...
async function loadDashboard() {
  ...
  try {
    ...
    actions.value = (histData.actions || []).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
...
async function submitQuickRec() {
  if (!recForm.studentName || !recForm.message) return
  sendingRec.value = true
  try {
    await createProfessionalRecommendation({
      studentId: recForm.studentName,
      message: recForm.message,
    })
...
.notif-mini-dot.dot-recommandation, .notif-mini-dot.dot-recommandation { background: var(--color-waiting-text, #A96F00); }
