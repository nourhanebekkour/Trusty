      ...
      <div v-for="(n, i) in filtered" :key="n.id" class="prof-notif-card" :class="{ 'prof-notif-unread': !n.isRead }" :style="{ animationDelay: i * 45 + 'ms' }">
        ...
        <div class="prof-notif-content">
          <div class="prof-notif-header">
            <span class="prof-notif-title">{{ sanitizeText(n.title) }}</span>
          ...
          <p class="prof-notif-message">{{ sanitizeText(n.message) }}</p>
          <div class="prof-notif-footer">
            ...
            <div class="prof-notif-footer-actions">
              ...
              <button class="btn-hide" @click.stop="hideOne(n)" :disabled="hidingId === n.id">
...
const ALLOWED_TYPES = ['VALIDATION', 'RECOMMANDATION', 'COMMENTAIRE', 'PROJET', 'STAGE', 'SYSTEME', 'MESSAGE', 'RAPPORT', 'COMPTE', 'SUGGESTION']
...
const tabs = computed(() => {
  ...
  const types = [...new Set(all.map(n => safeType(n.type)).filter(Boolean))]
  return [
...
const filtered = computed(() => {
  ...
  if (!ALLOWED_TYPES.includes(activeTab.value)) return all
  return all.filter(n => safeType(n.type) === activeTab.value)
})
...
function tabCount(key) {
  ...
  if (!ALLOWED_TYPES.includes(key)) return 0
  return notifications.value.filter(n => safeType(n.type) === key).length
}
...
function typeLabel(type) {
  ...
  return map[safeType(type)] || 'Général'
}
...
async function markOneAsRead(n) {
  if (!isValidNotification(n)) return
  if (markingId.value === n.id) return
...
async function hideOne(n) {
  if (!isValidNotification(n)) return
  if (hidingId.value === n.id) return
  hidingId.value = n.id
  try {
    await markProfessionalNotificationAsRead(n.id)
    notifications.value = notifications.value.filter(x => x.id !== n.id)
  } catch { showToast('Erreur') }
  finally   { hidingId.value = null }
}
